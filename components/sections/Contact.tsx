  ...
  return (
    <section id="contact" className="bg-white py-20">
      <div className="section-shell grid gap-8 lg:grid-cols-[0.8fr_1fr]">
        ...
        <MotionReveal className="card grid gap-4 p-6 sm:grid-cols-3">
          <a href={`tel:${settings.supportNumber}`} className="rounded-2xl bg-canvas p-5">
          ...
          <a href={`mailto:${settings.email}`} className="rounded-2xl bg-canvas p-5">
            <Mail className="mb-4 h-6 w-6 text-primary" aria-hidden="true" />
            <p className="text-sm font-semibold text-muted">Email</p>
            <p className="mt-2 break-words font-heading font-bold text-secondary">{settings.email}</p>
          </a>
